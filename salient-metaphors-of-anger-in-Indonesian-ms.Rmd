...
- expressing anger is expelling/releasing/channeling substance out (at others) (type = `r get_metaphor_mapping_n_lu(mapping_containedfluid_token_stats, 'releasing')`; token = `r get_metaphor_mapping_tokenfreq(mapping_containedfluid_token, 'releasing', lu_output = FALSE)`)

  - `r mapping_containedfluid_token %>% filter(status == 'no') %>% filter(MAPPING == "expressing anger is expelling/releasing/channeling substance out (at others)") %>% count(LU, LU_GLOSS) %>% mutate(lu_gloss_n = paste("*", str_replace_all(LU, "<\\/?w>", ""), "* '", LU_GLOSS, "' (", n, ")", sep = "")) %>% arrange(desc(n)) %>% pull(lu_gloss_n) %>% paste(collapse = "; ")`
...
- cause of anger is source of the liquid (type=`r get_metaphor_mapping_n_lu(mapping_containedfluid_token_stats, '^cause of anger')`; token=`r get_metaphor_mapping_tokenfreq(mapping_containedfluid_token, 'cause of anger', lu_output = FALSE)`)
  ...
  - `r get_lu_gloss_n_printed(mapping_containedfluid_token, '^cause of anger')`
...
- channeling anger is redirecting weapon to constructive goal (type=`r get_metaphor_mapping_n_lu(mapping_weapon_token_stats, "constructive")`; token=`r get_metaphor_mapping_tokenfreq(mapping_weapon_token, "constructive", lu_output = FALSE)`)
